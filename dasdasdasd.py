def rysunek_do_szyfrowany():
    ...
    bitowa_reprezentacja1 = [byte for byte in image_binary]
...
def rysunek_szyfrowany(x):
    ...
    with open(image_path, "rb") as image_file:
        image_binary = image_file.read()
    bitowa_reprezentacja0 = [byte for byte in image_binary]
    licznik = 0
    licznik2 = 0
    ...
    with open("C:\\tmp\\zaszyfrowany_obrazek.bmp", "wb") as output_file:
        output_file.write(bytearray(bitowa_reprezentacja0))

    with open("C:\\tmp\\gownodsadasd_psa.txt", "w") as txt_file:
        # Zapisuje bitową reprezentację jako tekst
        txt_file.write(
            ' '.join([f"{byte:02X}" for byte in bitowa_reprezentacja0]))
