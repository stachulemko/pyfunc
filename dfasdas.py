...
try:
    while page_counter < 50:
        results = 'http://groups.google.com/groups?q=' + \
            str(domain_name)+'&hl=en&lr=&ie=UTF-8&start=' + \
...
try:
    while page_counter_web < 50:
        results_web = 'http://www.google.com/search?q=%40' + \
            str(domain_name)+'&hl=en&lr=&ie=UTF-8&start=' + \
            repr(page_counter_web) + '&sa=N'
        request_web = urllib2.Request(results_web)
        request_web.add_header(
            'User-Agent', 'Mozilla/4.0 (compatible; MSIE 5.5; Windows NT 5.0)')
        opener_web = urllib2.build_opener()
        text = opener_web.open(request_web).read()
        emails_web = (re.findall(
            '([\w\.\-]+@'+domain_name+')', StripTags(text)))
